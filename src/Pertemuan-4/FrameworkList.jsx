        ...
        <div className="p-8">
            {frameworkData.map((item) => (
                <div key={item.id} className="border p-4 mb-4 rounded-lg shadow-md bg-white">
                    ...
                    <p className="text-blue-500 bg-gray-300 w-fit h-5 items-center p-2 rounded-2xl">
                        {item.details.developer}
                        <span className="font-bold" >({item.details.releaseYear})</span>
                    </p>
                    <a href={item.details.officialWebsite} className="text-blue-500 underline" target="_Blank">Visit Website</a>
